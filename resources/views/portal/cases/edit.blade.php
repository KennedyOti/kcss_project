                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('cases.update', $case->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="4" required>{{ old('description', $case->description) }}</textarea>
                            </div>
                                ...
                                <select class="form-select" id="assigned_staff_id" name="assigned_staff_id" required>
                                    ...
                                    @foreach ($staffMembers as $staff)
                                        <option value="{{ $staff->id }}"
                                            {{ old('assigned_staff_id', $case->assigned_staff_id) == $staff->id ? 'selected' : '' }}>
                                            {{ $staff->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="resolved"
                                        {{ old('status', $case->status) == 'resolved' ? 'selected' : '' }}>Resolved</option>
                                </select>
                            </div>
